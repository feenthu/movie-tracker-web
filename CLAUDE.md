...
This is a Next.js 15 movie tracking application using the App Router pattern. The architecture follows these key patterns:

### Core Stack
- **Framework**: Next.js 15 with App Router (no Pages Router)
- **Language**: TypeScript with strict mode enabled
- **Styling**: Tailwind CSS with custom MovieTracker-inspired theme
- **GraphQL**: Apollo Client with Next.js app support
...
│   ├── auth/              # Authentication pages (login, callback, signup)
...
- Configured with Railway-deployed GraphQL API at `movie-tracker-api-production.up.railway.app/graphql`
- Includes authentication link with JWT token handling
- Error handling with automatic token removal on 401
- Custom cache policies for movies data

**Authentication Flow**:
- JWT token-based authentication stored in localStorage
- OAuth2 support for external providers
- Auto-redirect to login on authentication errors
- Context provider manages auth state across app

**Styling System**:
- Tailwind CSS with HSL color variables
- Dark mode support via CSS variables
- Custom MovieTracker theme colors (`movietracker-*` classes)
- Global styles in `app/globals.css`
- Component styles use utility classes, not separate CSS files
- Responsive grid layouts for movie displays
- Custom aspect ratios for movie posters
...
- **API Endpoint**: `https://movie-tracker-api-production.up.railway.app/graphql`
- **Authentication**: Bearer token in Authorization header
- **Image Handling**: TMDB images configured in next.config.js
...
- **Component Tests**: Located alongside components in `__tests__` directories
- **Coverage**: Configured to collect from all `src/**` files
